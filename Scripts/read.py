...
if __name__ == "__main__":
    ...
    BOOK_PIC_PATH = get_book_pic_path(os.path.join(user_dir, "readbook/config.json"))

    parser = argparse.ArgumentParser(description="Book Reader")
    parser.add_argument('read', nargs='?', default=None, help="Chapter index to read")
    ...
    if args.read is None:
        chapters_list = book.getChapterName()
        current_position = book.CPosition
        # 计算起始和结束索引
        start_index = max(0, current_position - 5)
        end_index = min(len(chapters_list) - 1, current_position + 16)
        # 打印书签前5章和后15章
        print(BOOK_PIC_PATH)
        for i in range(start_index, end_index):
            if i == current_position:
                print(f"------> {i}: {chapters_list[i]} <------ (书签.)")  # 当前章节
            else:
                print(f"\t{i}: {chapters_list[i]}")
    else:
        try:
            chapter_index = int(args.read)
            book.read(chapter_index)
        except ValueError:
